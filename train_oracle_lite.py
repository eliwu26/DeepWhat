...
class OracleLiteDataset(Dataset):
    def __init__(self, tokens, question_lengths, features, categories, answers):
        ...
        features_crop_removed = np.hstack([
            features[:, :RESNET_FEATURE_SIZE],
            features[:, -SPATIAL_SIZE:]
        ])
        
        self.tokens = torch.from_numpy(tokens)
        self.question_lengths = torch.from_numpy(question_lengths)
        self.features = torch.from_numpy(features_crop_removed)
...
def get_data_loader(split, small):
    return DataLoader(
        OracleLiteDataset(*load_dataset(split, small)),
        batch_size=64,
        shuffle=True,
        num_workers=1
    )
...
def main():
    file_descriptor = 'oraclelite_gru2_fc2_cat32_h128_we64'
    small = False
